...
def bake_mesh_texture(
    ...
    Args:
        ...
        super_resolve: In ``"atlas"`` mode, solve for the texture whose
            reprojection best explains every view rather than blending them or
            picking one -- see :func:`bake_texture_atlas_super_resolved`.
            Strictly better than blending on both sharpness and pointwise
            error; sharper than ``view_selection`` but *not* strictly better
            than it, so it is opt-in. Not combinable with ``view_selection``
            (two answers to the same question) or ``num_pages > 1``.
    ...
    if super_resolve and view_selection:
        raise ValueError(
            ...
            "the same question -- which view, or which combination, should "
            "colour a texel -- and cannot both be applied. View selection is "
            "sharper than blending but pointwise worse; super-resolution is "
            "better than blending on both counts. Pick one."
        )
...
def bake_texture_atlas_super_resolved(
    ...
    **Cost grows as the cube of the atlas side, not the square.** Each CG
    matvec applies one masked Gaussian blur per view, and the PSF's width in
    texels grows with the atlas, so the kernel radius grows with it too:
    area x radius ~ N^3. Measured on a ten-view 96px capture of the test
    sphere, against the blended bake of the same mesh:

    ======  =======  ==========  ==============
    Atlas   Blended  Super-res   Times blended
    ======  =======  ==========  ==============
    128     0.3 s    1.0 s       3x
    256     0.4 s    11.0 s      25x
    512     1.0 s    85.2 s      88x
    1024    3.1 s    893.4 s     293x
    ======  =======  ==========  ==============

    At the CLI's default ``--texture_size 2048`` that extrapolates to hours,
    which is why this warns above ``warn_above_texels``. Size the atlas to the
    evidence (:func:`recommended_texture_size`) instead: a deconvolution
    cannot invent detail the photographs do not contain, so a larger atlas
    costs enormously more and recovers nothing.
    ...
    Args:
        ...
        warn_above_texels: Warn when ``texture_size`` exceeds this, because the
            solve's cost is cubic in it. 0 disables the warning.
    ...
    if warn_above_texels and texture_size > warn_above_texels:
        warnings.warn(
            f"Super-resolving a {texture_size}x{texture_size} atlas will be "
            "slow: the solve costs roughly the cube of the atlas side -- "
            "measured 85s at 512 and 893s at 1024 on a ten-view capture, so "
            "2048 runs into hours. Size the atlas to the evidence "
            "(recommended_texture_size) rather than to a round number: a "
            "deconvolution cannot recover detail the photographs do not "
            "contain, so a larger atlas costs far more and returns nothing.",
            RuntimeWarning,
            stacklevel=2,
        )
