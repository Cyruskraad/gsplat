...
class _MeshRenderedViews:
    """Views ray-cast from `mesh` itself, so geometry is not a confound.

    `_SphereDataset` ray-traces the *analytic* sphere while `_unit_sphere_mesh`
    is a polyhedron inscribed in it, and the gap between them is a sizeable
    ...
    for tessellation error it cannot fix, and the PSF it solves for would
    absorb the difference.

    Reuses `examples/make_synthetic_capture.py`'s camera helpers, which also
    pins them: if that example's pose convention breaks, this fails rather than
    the example silently rotting.
    """

    def __init__(self, mesh, num_views=10, width=48):
        import sys
        from pathlib import Path
        ...
        from make_synthetic_capture import fibonacci_directions, look_at
        ...
        rows, cols = np.meshgrid(np.arange(width), np.arange(width), indexing="ij")
        # +0.5 puts the ray through the pixel centre; the projection convention
        # elsewhere has integer coordinates at pixel corners.
        x = (cols - K[0, 2] + 0.5) / K[0, 0]
        y = (rows - K[1, 2] + 0.5) / K[1, 1]
        dirs_cam = np.stack([x, y, np.ones_like(x)], axis=-1)
        dirs_cam /= np.linalg.norm(dirs_cam, axis=-1, keepdims=True)

        self._items = []
        for direction in fibonacci_directions(num_views):
            camtoworld = look_at(3.5 * direction, np.zeros(3))
            dirs_world = np.einsum("ij,hwj->hwi", camtoworld[:3, :3], dirs_cam)
            origin = camtoworld[:3, 3]
            rays = np.concatenate(
                [np.broadcast_to(origin, dirs_world.shape), dirs_world], axis=-1
            ).astype(np.float32)
            hit = scene.cast_rays(o3d.core.Tensor(rays.reshape(-1, 6)))
            t_hit = hit["t_hit"].numpy().reshape(width, width)
            visible = np.isfinite(t_hit)
            points = origin[None, None, :] + dirs_world * t_hit[..., None]

            image = np.zeros((width, width, 3))
            image[visible] = _high_frequency_pattern(points[visible])
            image = (np.clip(image, 0.0, 1.0) * 255.0).round()
            self._items.append(
                {
                    "camtoworld": torch.from_numpy(np.ascontiguousarray(camtoworld)),
                    "K": torch.from_numpy(K),
...
def _mesh_rendered_views(mesh, num_views=10, width=48):
    pytest.importorskip("torch", reason="torch not installed")
    return _MeshRenderedViews(mesh, num_views=num_views, width=width)
